...
def parse_cmdline_options():
    ...
    parser.add_option(
        ...
        default=False,
        dest="addproject",
        help="Add new project to baremetal projects",
    )
    parser.add_option(
        "-d",
        "--del",
        ...
        default=False,
        dest="delproject",
        help="Delete existing project from baremetal projects",
    )
    parser.add_option(
        "-i",
        "--desc",
    ...
    if options.delproject:
        ...
        if options.projdesc or not options.srcpath:
            parser.error(
                "--desc provided or --src missing with -d, use -h argument for help"
            )
            exit()
...
def container_cml_templ_add_symbl(projdesc, projname):
    ...
    pattern = re.compile(r"(CONT\%\(cn\)d_BAREMETAL_PROJ_)(.*)")
    ...
    with open(cont_templ, "r") as fin:
        ...
        for line in fin:
            ...
            # Search for baremetal menu and add new project symbol
                ...
                len(parts) == 1
                and parts[0] == baremetal_name_templ + last_baremetal_proj
            ):
...
def add_project(projdesc, srcdir, projname):
    ...
    print("Done, New baremetal project " + projname + " is ready to be used.")
