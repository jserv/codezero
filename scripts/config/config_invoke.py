...
def autogen_rules_file(options, args):
    ...
    if not options.ncont:
        options.ncont = 4
        print(
            "Max container count not supplied (-n), using %d as default."
            % options.ncont
        )
...
def cml2_header_to_symbols(cml2_header, config):
    with open(cml2_header) as header_file:
...
def build_parse_options():
    ...
    parser.add_option(
        "-a", "--arch", type="string", dest="arch", help="Specify architecture."
    )
    ...
    parser.add_option(
        "-f", "--file", dest="cml_file", help="Specify user-defined configuration file."
    )

    parser.add_option(
        "-C", "--configure", action="store_true", dest="config", help="Configure only."
    )
...
def configure_system(options, args):
    ...
    #
    if (
        options.cml_file or os.path.exists(CML2_CONFIG_FILE)
        ...
        # Compile rules file.
        os.system(
            CML2TOOLSDIR + "/cmlcompile.py -o " + CML2_COMPILED_RULES + " " + rules_file
        )
    ...
    # Create header file
    os.system(
        TOOLSDIR + "/cml2header.py -o " + CML2_CONFIG_H + " -i " + CML2_CONFIG_FILE
    )
...
def rename_config_cml(options):
    ...
    new_cml_file = join(
        BUILDDIR, strftime("%a-%d%b%Y-%H:%M:%S", localtime()) + "-" + config.platform
    )
