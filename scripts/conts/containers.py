...
def fill_pager_section_markers(cont, pager_binary):
    (
        cont.pager_rw_pheader_start,
        cont.pager_rw_pheader_end,
        cont.pager_rx_pheader_start,
        cont.pager_rx_pheader_end,
...
def build_linux_container(config, projpaths, container, opts):
    ...
    pager_binary = join(
        BUILDDIR, "cont" + str(container.id) + "/linux/kernel-2.6.34/linux.elf"
    )
    config.containers[container.id].pager_size = conv_hex(elf_binary_size(pager_binary))
...
def glob_by_walk(builddir, ext, imglist):
    """Walk directory tree and collect files matching extension pattern."""
    for dirpath, dirnames, filenames in os.walk(builddir):
        files = glob.glob(join(dirpath, ext))
...
def build_posix_container(config, projpaths, container, opts):
    ...
    glob_by_walk(builddir, "*.elf", images)

    # Calculate and store size of pager
    pager_binary = join(BUILDDIR, "cont" + str(container.id) + "/posix/mm0/mm0.elf")
    config.containers[container.id].pager_size = conv_hex(elf_binary_size(pager_binary))
...
def build_all_containers(opts):
    ...
    for container in config.containers:
        if container.type == "linux":
            cont_images.append(
                build_linux_container(config, projpaths, container, opts)
            )
        elif container.type == "baremetal":
            cont_images.append(
                build_default_container(config, projpaths, container, opts)
            )
        elif container.type == "posix":
            cont_images.append(
                build_posix_container(config, projpaths, container, opts)
            )
        else:
            print(
