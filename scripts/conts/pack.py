...
class LinuxContainerPacker:
    def __init__(self, container, linux_builder):
        ...
        self.CONTAINER_BUILDDIR_BASE = source_to_builddir(
            join(PROJROOT, "conts"), container.id
        )

        self.container_lds_out = join(self.CONTAINER_BUILDDIR_BASE, "container.lds")
        self.container_S_out = join(self.CONTAINER_BUILDDIR_BASE, "container.S")
        self.container_elf_out = join(
            self.CONTAINER_BUILDDIR_BASE, "container" + str(container.id) + ".elf"
        )

        self.kernel_image_in = linux_builder.kernel_image
        self.rootfs_elf_in = join(
            self.CONTAINER_BUILDDIR_BASE, "linux/rootfs/rootfs.elf"
        )
    ...
    def pack_container(self, config):
        self.generate_container_lds(
            [self.kernel_image_in, self.rootfs_elf_in, self.atags_elf_in]
        )
        self.generate_container_assembler(
            [self.kernel_image_in, self.rootfs_elf_in, self.atags_elf_in]
        )
        os.system(
            ...
            + "gcc "
            + "-nostdlib -o %s -T%s %s"
            % (self.container_elf_out, self.container_lds_out, self.container_S_out)
        )
...
class DefaultContainerPacker:
    def __init__(self, container, images_in):
        ...
        # Here, we simply attempt to get PROJROOT/conts as
        # PROJROOT/build/cont[0-9]
        self.CONTAINER_BUILDDIR_BASE = join(
            source_to_builddir(join(PROJROOT, "conts"), container.id), "packer"
        )
