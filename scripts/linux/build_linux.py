...
class LinuxUpdateKernel:
    def __init__(self, container):
        ...
        self.system_macros_h_out = join(
            LINUX_KERNELDIR, "arch/arm/include/vmm/system_macros.h"
        )
        self.system_macros_h_in = join(
            LINUX_KERNELDIR, "arch/arm/include/vmm/system_macros.h.in"
        )
...
class LinuxBuilder:
    def __init__(self, pathdict, container, opts):
        ...
        self.LINUX_KERNEL_BUILDDIR = source_to_builddir(LINUX_KERNELDIR, container.id)

        self.container = container
        self.kernel_binary_image = join(
            os.path.relpath(self.LINUX_KERNEL_BUILDDIR, LINUX_KERNELDIR), "vmlinux"
        )
    ...
    def defconfig_to_config(self, config):
        ...
        if config.platform.upper() == "EB":
            ...
            self.platform_defconfig = self.platform_config_files[
                config.platform.upper()
            ]

        if not self.platform_defconfig:
            print("Platform detected as: " + config.platform)
            print(
                "Could not find relevant linux config file please review configuration"
            )
            sys.exit(1)
    ...
    def build_linux(self, config, opts):
        ...
        if opts.batch:
            ...
            if not os.path.exists(self.build_config_file):
                self.print_verbose(
                    "Config file doesnt exist after building: " + self.build_config_file
                )
                sys.exit(1)
        ...
        linux_elf_gen_cmd = (
            ...
            + str(
                conv_hex(
                    -self.container.linux_page_offset + self.container.linux_phys_offset
                )
            )
            + " "
