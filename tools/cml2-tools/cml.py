...
class ConfigSymbol:
    ...
    def __init__(
        self, name, type=None, default=None, prompt=None, file=None, lineno=None
    ):
...
def evaluate(exp, debug=0):
    ...
    if type(exp) is type(()):
        ...
        # Logical operations -- always trit-valued
            ...
            return tritify(
                not ((evaluate(exp[1], debug) and not evaluate(exp[2], debug)))
            )
        elif exp[0] == "==":
    ...
    elif isinstance(exp, trit) or type(exp) in (type(""), type(0)):
        if debug > 2:
...
def display_expression(exp):
    ...
    if type(exp) is type(()):
        if exp[0] == "not":
            ...
            return "(%s ? %s : %s)" % (
                display_expression(exp[1]),
                display_expression(exp[2]),
                display_expression(exp[3]),
            )
        else:
            return "(%s %s %s)" % (
                display_expression(exp[1]),
