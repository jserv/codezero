...
import os, string, getopt, cmd, time, random
    ...
    if level >= 4:
        if room.type == "choices":
            ...
            for symbol in room.items:
                if (
                    symbol.is_symbol()
                    and configuration.is_visible(symbol)
                    and not symbol.eval()
                ):
...
class advent_menu(cmd.Cmd):
    ...
    def do_look(self, line):
        ...
        # Look at an option
            ...
            lambda x: x.name, filter(lambda x: x.is_logical(), self.current.items)
        ):
    ...
    def do_nearby(self, dummy):
        if self.current != configuration.start:
            print(
                lang["ROOMBANNER"] % (self.current.menu.name, self.current.menu.prompt)
            )
        for dir, symbol in self.current.directions.items():
            if symbol.type in ("menu", "choices") and configuration.is_visible(symbol):
                print(
                    ("%-2s: " % dir) + lang["ROOMBANNER"] % (symbol.name, symbol.prompt)
                )
        if len(self.current.exits) > len(directions):
            print(
                lang["EXTRAROOMS"]
                % ", ".join(map(lambda x: x.name, self.current.exits[9:]))
            )
        print
    ...
    def do_ne(self, dummy):
        self.do_dir("ne")

    def do_nw(self, dummy):
        self.do_dir("nw")

    def do_se(self, dummy):
        self.do_dir("se")

    def do_sw(self, dummy):
        self.do_dir("sw")
    ...
    def do_drop(self, line):
        ...
        if words == ["all"] and self.current.type != "choices":
            words = map(
                lambda x: x.name,
                filter(
                    lambda x: x.is_logical()
                    and configuration.is_visible(x)
                    and not x.eval(),
                    self.current.items,
                ),
            )
...
def process_define(myconfiguration, val, freeze):
    ...
    if sym.is_derived():
        ...
        val = parts[1]
    ok, effects, violations = myconfiguration.set_symbol(
