...
class Token:
    ...
    def __lt__(self, other):
        if isinstance(other, Token):
            ...
            return self.attr < other.attr if self.attr else False
        else:
            return self.type < other
...
class lexwrapper(shlex.shlex):
    ...
    def demand(self, type, attr=None):
        ...
        if tok.type == "EOF":
            ...
            self.croak(
                "syntax error, expecting token of type `%s' (actually saw %s=%s)"
                % (type, tok.type, tok.attr)
            )
        else:
            return tok.attr
...
def parse_atom(input):
    if compstate.debug >= 2:
        print("entering parse_atom...")
    op = input.lex_token()
    if op.type in _atoms:
        if compstate.debug >= 2:
            print("parse_atom returns", op.attr)
        return op
    elif op.type == "(":
        ...
        if close != ")":
            ...
            if compstate.debug >= 2:
                print("parse_atom returns singleton", sub)
            return sub
...
def parse_term(input):
    if compstate.debug >= 2:
        print("entering parse_term...")
    left = parse_atom(input)
    op = input.lex_token()
    if op.type not in _termops:
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_term returns singleton", left)
        return left
    right = parse_term(input)
    expr = (op.type, left, right)
    if compstate.debug >= 2:
        print("parse_term returns", expr)
    return expr


def parse_relational(input):
    if compstate.debug >= 2:
        print("entering parse_relational...")
    left = parse_term(input)
    op = input.lex_token()
    if op.type not in _relops:
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_relational returns singleton", left)
        return left
    right = parse_term(input)
    expr = (op.type, left, right)
    if compstate.debug >= 2:
        print("parse_relational returns", expr)
    return (op.type, left, right)


def parse_assertion(input):
    if compstate.debug >= 2:
        print("entering parse_assertion...")
...
def parse_conjunct(input):
    if compstate.debug >= 2:
        print("entering parse_conjunct...")
    left = parse_assertion(input)
    op = input.lex_token()
    if op.type != "and":
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_conjunct returns singleton", left)
        return left
    else:
        expr = ("and", left, parse_conjunct(input))
        if compstate.debug >= 2:
            print("parse_conjunct returns", expr)
        return expr


def parse_disjunct(input):
    if compstate.debug >= 2:
        print("entering parse_disjunct...")
    left = parse_conjunct(input)
    op = input.lex_token()
    if op.type != "or":
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_disjunct returns singleton", left)
        return left
    else:
        expr = ("or", left, parse_disjunct(input))
        if compstate.debug >= 2:
            print("parse_disjunct returns", expr)
        return expr
...
def parse_factor(input):
    ...
    if op.type != "implies":
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_factor returns singleton", left)
        return left
    else:
        expr = ("implies", left, parse_disjunct(input))
        if compstate.debug >= 2:
            print("parse_factor returns", expr)
        return expr


def parse_summand(input):
    if compstate.debug >= 2:
        print("entering parse_summand...")
    left = parse_factor(input)
    op = input.lex_token()
    if op.type != "*":
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_summand returns singleton", left)
        return left
    else:
        expr = ("*", left, parse_expr_inner(input))
        if compstate.debug >= 2:
            print("parse_summand returns", expr)
        return expr


def parse_ternary(input):
    if compstate.debug >= 2:
        print("entering parse_ternary...")
    guard = parse_summand(input)
    op = input.lex_token()
    if op.type != "?":
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_ternary returns singleton", guard)
        return guard
    else:
        ...
        if compstate.debug >= 2:
            print("parse_ternary returns", expr)
        return expr


def parse_expr_inner(input):
    if compstate.debug >= 2:
        print("entering parse_inner_expr...")
    left = parse_ternary(input)
    op = input.lex_token()
    if op.type not in ("+", "-"):
        input.push_token(op)
        if compstate.debug >= 2:
            print("parse_expr_inner returns singleton", left)
        return left
    else:
        expr = (op.type, left, parse_expr_inner(input))
        if compstate.debug >= 2:
            print("parse_expr_inner returns", expr)
        return expr
...
def intern_symbol(input, name=None, oktypes=None, record=0):
    ...
    if name not in rulebase.dictionary:
        ref = rulebase.dictionary[name] = cml.ConfigSymbol(
            name, None, None, None, input.infile, input.lineno
        )
        ...
        if ref.type and oktypes is not None and ref.type not in oktypes:
            input.complain(
                'incompatible previous declaration of %s as %s (see "%s", %d)'
                % (name, ref.type, ref.file, ref.lineno)
            )
        if record:
            if record:
                ...
                input.complain(
                    'duplicate symbol %s (see "%s", line %d)'
...
def parse(input, baton):
    ...
    while 1:
        ...
        if compstate.debug > 1:
            print("Parsing declaration beginning with %s..." % (leader,))
        ...
        if leader.type == "EOF":
            ...
            input.complain(
                "menus and explanations declarations are "
            ...
            while 1:
                ...
                if tok.type == "text":
                    ...
                    if target.attr not in rulebase.dictionary:
                        ...
                        rulebase.dictionary[ref.name].helptext = rulebase.dictionary[
                            target.attr
                        ].help()
                else:
                    input.push_token(tok)
            ...
            for symbol in list:
                if symbol.menu:
                    input.complain(
                        "symbol %s in %s occurs in another menu (%s)"
            ...
            items_list = parse_symbol_tree(input)
            for symbol in items_list:
                symbol.type = "bool"
                symbol.choicegroup = [x for x in items_list if x != symbol]
            dflt = input.lex_token()
            if dflt.type != "default":
                default = items_list[0].name
                input.push_token(dflt)
            else:
                default = intern_symbol(input, None, None, record=1)
            if default not in items_list:
                input.complain("default %s must be in the menu" % (repr(default),))
            else:
                menusym.default = default
                menusym.items = items_list
                for symbol in items_list:
                    if symbol.menu:
                        input.complain(
                            "symbol %s occurs in another menu (%s)"
            ...
            if next.type != "explanation":
                ...
                if expl.type != "word":
                    input.complain(
                        "while expecting a word of explanation, I see %s"
                        % (repr(expl),)
                    )
                    continue
                ...
                msg = entry.prompt
            rulebase.constraints.append(
                cml.Requirement(expr, msg, input.infile, input.lineno)
            )
            ...
            if next.type == "range":
                ...
                while 1:
                    ...
                    if low.type in _keywords:
                        ...
                        input.complain(
                            "bad token %s where range literal expected" % (low.attr)
                        )
                ...
                while 1:
                    ...
                    if name.type in _keywords:
                        ...
                        input.complain(
                            "bad token %s where enum name expected" % (name.attr)
                        )
                        continue
...
def get_symbol_declaration(input):
    ...
    global compstate
    if compstate.debug >= 2:
        print("entering get_symbol_declaration...")
    props = []
    propflag = 1
    while 1:
            ...
            for flag, property in level:
                if flag:
                    ...
                    if property not in propdict:
                        input.complain(
                            "property %s can't be removed when it's not present"
                            % property
                        )
                    else:
                        propdict[property] = 0
...
def parse_symbol_tree(input):
    global compstate
    if compstate.debug >= 2:
        print("entering parse_symbol_tree...")
...
def traverse_make_dep(symbol, guard, input):
    ...
    if symbol in compstate.derivations:
        return
    elif isinstance(guard, cml.trit) or (
        isinstance(guard, Token) and guard.attr in ("n", "m", "y")
    ):
        return
    elif isinstance(guard, Token):
        if guard in compstate.explicit_ancestors[symbol]:
            input.complain(
                "%s is already an ancestor of %s" % (guard.attr, symbol.name)
            )
        else:
        ...
        if guard in compstate.explicit_ancestors[symbol]:
            input.complain(
                "%s is already an ancestor of %s" % (guard.name, symbol.name)
            )
        else:
            compstate.explicit_ancestors[symbol].append(guard)
    ...
    if isinstance(expr, cml.ConfigSymbol):
        ...
        if left != right:
            simple_error(
                file,
                line,
                "types %s and %s don't match in ternary expression\n" % (left, right),
            )
        return left
        ...
        if left not in ("integer", "trit"):
            simple_error(
                file,
                line,
                "bad %s left operand for arithmetic operator %s\n" % (left, expr[0]),
            )
        ...
        if right not in ("integer", "trit"):
            simple_error(
                file,
                line,
                "bad %s right operand for arithmetic operator %s\n" % (right, expr[0]),
            )
        return "integer"
        ...
        if left != "trit":
            simple_error(
                file,
                line,
                "bad %s left operand for trit operator %s\n" % (left, expr[0]),
            )
        right = validate_expr(expr[2], file, line)
        if right != "trit":
            simple_error(
                file,
                line,
                "bad %s right operand for trit operator %s\n" % (right, expr[0]),
            )
        return "trit"
        ...
        if left != right:
            simple_error(
                file,
                line,
                "types %s and %s don't match in %s expression\n"
                % (expr[0], left, right),
            )
        return "trit"
    else:
...
def ancestry_check(symbol, counts):
    ...
    for ancestor in symbol.ancestors:
        if ancestor.name in counts:
            ...
            list(
                map(
                    lambda symbol, counts=counts: ancestry_check(symbol, counts),
                    symbol.ancestors,
                )
            )
...
def compile(debug, arguments, profile, endtok=None):
    ...
    for ref in rulebase.dictionary.values():
        if not ref.prompt and ref not in compstate.derivations:
            postcomplain(
                '"%s", line %d: %s in menu %s has no prompt\n'
    ...
    rulebase.constraints = list(
        map(
            lambda x: cml.Requirement(resolve(x.predicate), x.message, x.file, x.line),
            rulebase.constraints,
        )
    )
    if compstate.bad_symbols:
        postcomplain(
            "%d symbols could not be resolved:\n" % (len(compstate.bad_symbols),)
        )
                ...
                if entry.default == cml.m:
                    ...
                    if entry.default[0] in _boolops + _relops:
                        ...
                        if isinstance(entry.default[2], cml.ConfigSymbol):
                            ...
                            derived_type = "trit"
                        elif type(entry.default[2]) is int:
                            derived_type = "decimal"
                        elif type(entry.default[2]) is str:
                            derived_type = "string"
                elif type(entry.default) is int:
                    derived_type = "decimal"  # Could be hex
                elif type(entry.default) is str:
                    derived_type = "string"
    ...
    for entry in rulebase.dictionary.values():
        if entry in compstate.derivations and not entry.type:
            postcomplain(
                error_leader(entry.file, entry.lineno)
    ...
    for i in range(len(preorder)):
        ...
        if forwards:
            ...
            postcomplain(
                '"%s", line %d: %s in %s requires %s forward\n'
    ...
    for entry in rulebase.dictionary.values():
        ...
        if entry in compstate.derivations:
            if entry.menu:
                postcomplain(
                    "menu %s contains derived symbol %s\n"
        ...
        if entry.is_symbol() and not entry.default is None:
            if type(entry.default) is int and not entry.is_numeric():
                postcomplain(
                    "%s is not of numeric type but has numeric constant default\n"
                    % entry.name
                )
            elif type(entry.default) is str and not entry.type == "string":
                postcomplain(
                    "%s is not of string type but has string constant default\n"
                    % entry.name
                )
        ...
        if entry.type in ("decimal", "hexadecimal", "string"):
            ...
            entry.discrete = not list(filter(lambda x: type(x) is tuple, entry.range))
            # This member can be used by front ends to determine whether the
            # entry's value should be queried with a pulldown of enums.
            entry.enum = (
                type(entry.range[0]) is tuple and type(entry.range[0][0]) is str
            )
    ...
    if not compstate.errors:
        for wff in rulebase.constraints:
            if not cml.evaluate(wff.predicate, debug):
                postcomplain(
                    error_leader(wff.file, wff.line)
    ...
    if debug:
        ...
        for entry in rulebase.dictionary.values():
            if (
                not entry.type in ("message", "menu", "choices", "explanation")
                and entry.prompt
                and not entry.help()
            ):
        ...
        if orphans:
            postcomplain(
                "The following help entries do not correspond to symbols: %s\n"
                % orphans
            )
    if not debug and not compstate.errors:
        baton.end("Done")
    ...
    if compstate.errors:
        ...
        rulebase.reduced = list(map(lambda x: x.predicate, rulebase.constraints))
        rulebase.optimize_constraint_access()
        if debug:
            cc = dc = tc = 0
            for symbol in rulebase.dictionary.values():
                if symbol not in compstate.derivations:
                    tc = tc + 1
                ...
                if symbol.constraints:
                    cc = cc + 1
            print(
                "%d total symbols; %d symbols are involved in constraints; %d in dependencies."
                % (tc, cc, dc)
            )
...
if __name__ == "__main__":
    ...
    def main(debug, outfile, arguments, profile):
        ...
        if not rulebase:
            ...
            try:
                if debug:
                    print("cmlcompile: output directed to %s" % (outfile))
                out = open(outfile, "wb")
