...
class tty_style_base(cmd.Cmd):
    ...
    def do_y(self, line):
        if not line:
            ...
            if not line in configuration.dictionary:
                print(lang["NONEXIST"] % line)
                return
            else:
    ...
    def do_s(self, line):
        file = string.strip(line)
        failure = configuration.save(
            file, cml.Baton(lang["SAVESTART"] % file, lang["SAVEEND"])
        )
        if failure:
            print(failure)
    ...
    def generate_prompt(self, symbol):
        ...
        if symbol.type in ("bool", "trit"):
            ...
            return (
                ...
                + leader
                + "%s: %s %s%s: "
                % (symbol.name, cgenprompt(symbol), selected, havehelp)
            )
...
class debugger_style_menu(tty_style_base):
    ...
    def do_f(self, line):
        ...
        if not line:
            print(lang["NOSYMBOL"])
        elif not line in configuration.dictionary:
            print(lang["NONEXIST"] % line)
        else:
...
class MenuBrowser:
    ...
    def push(self, browseable, selected=None):
        ...
        if self.debug:
            self.errout.write(
                "MenuBrowser.push(): pushing %s=@%d, selection=%s\n"
        ...
        if selected == None:
            ...
            if self.debug:
                self.errout.write(
                    "MenuBrowser.push(): selection set to %d\n" % (selnum)
                )
        ...
        if self.debug:
            ...
            self.errout.write(
                "MenuBrowser.push(): pushed %s=@%d->%d, selection=%d, viewbase=%d\n"
        ...
        if not self.page_stack:
            ...
            if self.debug:
                if len(self.page_stack) == 0:
                    ...
                    self.errout.write(
                        "MenuBrowser.pop(): new level %d, object=@%d, selection=%d, viewbase=%d\n"
                        % (
                            len(self.page_stack),
                            id(self.page_stack[-1]),
                            self.selection_stack[-1],
                            self.viewbase_stack[-1],
                        )
                    )
            return item
    ...
    def list(self):
        ...
        if self.debug:
            self.errout.write(
                "MenuBrowser.list(): stack level %d. object @%d, listing %s\n"
                % (
                    len(self.page_stack) - 1,
                    id(object),
                    object[viewbase : viewbase + self.viewport_height],
                )
            )
    ...
    def selected(self):
        ...
        if self.debug:
            self.errout.write(
                "MenuBrowser.selected(): at %d, object=@%d, %s\n"
    ...
    def viewbase(self):
        ...
        if self.debug:
            self.errout.write(
                "MenuBrowser.viewbase(): at level=%d, object=@%d, %d\n"
                % (
                    len(self.page_stack) - 1,
                    id(object),
                    base,
    ...
    def goto(self, newloc):
        ...
        if self.debug:
            self.errout.write(
                "MenuBrowser.down(): at level=%d, object=@%d, old selection=%d, new selection = %d, new base = %d\n"
                % (
                    len(self.page_stack) - 1,
                    id(self.page_stack[-1]),
                    oldloc,
...
class PopupBaton:
    ...
    def __init__(self, startmsg, master):
        self.subwin = master.window.subwin(
            3,
            len(startmsg) + 3,
            (master.lines - 3) / 2,
...
class WindowBaton:
    ...
    def twirl(self, ch=None):
        if ch:
            ...
            self.master.window.addch(
                0, self.master.columns - 1, "-/|\\"[self.count % 4]
            )
...
class curses_style_menu:
    ...
    def __init__(self, stdscr, config, mybanner):
        ...
        if self.lines < 9 or self.columns < 60:
            raise "TERMTOOSMALL"
        self.menus.viewport_height = (
            self.lines
            - 2
            + (
                not configuration.expert_tie
                or cml.evaluate(configuration.expert_tie) != cml.n
            )
        )
    ...
    def help_popup(self, instructions, msglist, beep=1):
        ...
        for i in range(len(msgparts)):
            subwin.addstr(
                pad + i,
                pad + int((msgwidth - len(msgparts[i])) / 2),
                msgparts[i],
                curses.A_BOLD,
            )
        subwin.addstr(
            pad * 2 + len(msgparts) - 1, pad + int((msgwidth - len(leave)) / 2), leave
        )
        subwin.box()
        if beep:
            curses.beep()
    ...
    def set_symbol(self, sym, val, freeze=0):
        ...
        if ok:
            ...
            self.help_popup(
                "PRESSANY",
                effects + [lang["BADREQUIRE"]] + map(repr, violations),
                beep=1,
            )
    ...
    def recompute(self, here):
        ...
        visible = filter(
            lambda x, m=here: hasattr(m, "nosuppressions") or interactively_visible(x),
            here.items,
        )
    ...
    def redisplay(self, repaint):
        ...
        if self.banner and self.in_menu():
            title = (
                self.msgbuf
                + (" " * (self.columns - len(self.msgbuf) - len(self.banner) - 1))
                + self.banner
            )
        else:
        ...
        if self.in_menu():
            screenvals = self.values[
                self.menus.viewbase() : self.menus.viewbase()
                + self.menus.viewport_height
            ]
            ...
            if i < len(screenlines):
                ...
                if type(child) is type(""):
                    ...
                    self.window.hline(
                        ...
                        len(child.prompt) + 2,
                        curses.ACS_HLINE,
                        self.columns - len(child.prompt) - 3,
                    )
                else:
                    ...
                    leftpart = ("  " * child.depth) + cgenprompt(
                        child,
                        not configuration.expert_tie
                        or not cml.evaluate(configuration.expert_tie),
                    )
                    ...
                    if "edit" in repaint and child == self.menus.selected():
                        ...
                        self.window.addstr(
                            valstring
                            + (" " * (curses_style_menu.valwidth - len(valstring)))
                            + " "
                        )
    ...
    def symbol_menu_command(self, cmd, operand):
        ...
        if cmd == curses.KEY_LEFT:
            ...
            if not self.in_menu():
                self.help_popup("PRESSANY", (lang["NOSYMBOL"],))
            elif operand.type in ("bool", "trit") and operand.menu.type != "choices":
            ...
            try:
                ...
                if errors:
                    ...
                    self.help_popup(
                        "PRESSANY", [lang["INCCHANGES"] % (changes, file)], beep=0
                    )
                recompute = 1
        elif cmd == ord("I"):
            file = self.query_popup(lang["LOADFILE"])
            ...
            if pattern:
                try:
                    ...
                    self.help_popup("PRESSANY", (lang["SEARCHINVAL"], str(detail)))
                else:
            ...
            failure = configuration.save(config, PopupBaton(lang["SAVING"], self))
            if failure:
                self.help_popup("PRESSANY", [failure])
        else:
            self.help_popup("PRESSANY", (lang["UNKNOWN"] % (curses.keyname(cmd)),))
        return recompute

    def seek_mutable(self, direction, movefirst=0):
        if movefirst:
            self.menus.move(delta=direction, wrap=1)
        while self.menus.selected().type == "message" or self.menus.selected().frozen():
            self.menus.move(delta=direction, wrap=1)
        ...
        while 1:
            ...
            if "edithelp" in repaint:
                ...
                if cmd in (
                    ...
                    curses.ascii.CR,
                    curses.ascii.BEL,
                    ord(curses.ascii.ctrl("p")),
                    ord(curses.ascii.ctrl("n")),
                ):
                    if cmd != curses.ascii.BEL:
                        newval = self.textbox.gather()
                        self.accept_field(sel_symbol, newval, current_prompt)
                    ...
                    if sel_symbol.type == "decimal" and not curses.ascii.isdigit(cmd):
                        curses.beep()
                    elif sel_symbol.type == "hexadecimal" and not curses.ascii.isxdigit(
                        cmd
                    ):
                        curses.beep()
                    else:
                    ...
                    cmd == curses.KEY_RIGHT
                    and self.textbox.win.getyx()[1] >= curses_style_menu.valwidth
                ):
                    oldval = newval
                    ...
                    if newval:
                        ...
                        self.textbox.win.addstr(
                            0, 0, newval[0 : curses_style_menu.valwidth]
                        )
                        recompute = 1
                    self.textbox = None
                    repaint = ["main"]
                ...
                if cmd == curses.ascii.FF:
                    ...
                    if self.in_menu():
                        ...
                        self.msgbuf = (
                            lang["WELCOME"] % (configuration.banner)
                    ...
                    if not symname in configuration.dictionary:
                    ...
                    if not self.in_menu():
                        ...
                        if help:
                            ...
                            self.help_popup(
                                "PRESSANY", (lang["NOHELP"] % (sel_symbol.name,),)
                            )
                ...
                # Following commands are not executed in helpmode
                    ...
                    failure = configuration.save(
                        config, PopupBaton(lang["SAVING"], self)
                    )
                    if failure:
                        self.help_popup("PRESSANY", [failure])
                    break
                    ...
                    if sel_symbol.type == "message":
                        ...
                        while (
                            not interactively_visible(self.menus.selected())
                            or self.menus.selected().type == "message"
                        ):
                        ...
                        pass
                    elif sel_symbol.type == "bool" or (
                        sel_symbol.type == "trit" and not configuration.trits_enabled
                    ):
                        ...
                        win = curses.newwin(
                            1, curses_style_menu.valwidth + 1, current_line + 1, 1
                        )
                        self.textbox = curses.textpad.Textbox(win)
                        self.textbox.win.addstr(
                            0, 0, current_prompt[: curses_style_menu.valwidth]
                        )
                        newval = current_prompt
                        self.textbox.win.move(0, 0)
                        self.msgbuf = lang["EDITING"] % (
                            sel_symbol.name[: self.columns - 1],
                        )
                        repaint = ["main", "edit"]
                    recompute = 1
                else:
...
try:
    ...
    class ValidatedField(Frame):
        ...
        def error_popup(self, title, mybanner, text):
            ...
            Button(
                self.errorwin,
                text=lang["DONE"],
                command=lambda x=self.errorwin: Widget.destroy(x),
                bd=2,
            ).pack()
    ...
    class PromptGo(Frame):
        ...
        def __init__(self, master, label, command):
            ...
            self.promptframe.L = Label(self.promptframe, text=lang[label], anchor=W)
            ...
            Button(
                self.promptframe, text=lang["GO"], command=self.dispatch, bd=2
            ).pack()
    ...
    def makehelpwin(title, mybanner, text):
        ...
        Button(
            makehelpwin,
            text=lang["DONE"],
            command=lambda x=makehelpwin: x.destroy(),
            bd=2,
        ).pack()
        ...
        makehelpwin.textwidget.tag_bind(
            "url",
            "<Enter>",
            lambda event, x=makehelpwin.textwidget: x.config(cursor="hand2"),
        )
        makehelpwin.textwidget.tag_bind(
            "url",
            "<Leave>",
            lambda event, x=makehelpwin.textwidget: x.config(cursor="xterm"),
        )
            ...
            if interactively_visible(child):
                if child.type == "menu" and cmlnode.items:
                    ...
                    if child.type in ("trit", "bool") and (
                        ...
                        child.yes = "yes"
                    elif child.type in ("trit", "bool") and child.eval() == cml.n:
    ...
    class ConfigMenu(Frame):
        ...
        def __init__(self, menu, config, mybanner):
            ...
            self.helpmenu = self.makeMenu(
                lang["HELPBUTTON"], (("HELPBUTTON", self.cmdhelp),)
            )
            ...
            self.backbutton = Button(
                self.toolbar, text=lang["BACKBUTTON"], command=self.pop
            )
            self.backbutton.pack(side=LEFT)
            self.helpbutton = Button(
                self.toolbar,
                text=lang["HELPBUTTON"],
    ...
    class ConfigTreeMenu(ConfigMenu):
        ...
        def draw_optionframe(self):
            ...
            if node:
                ...
                if node.type == "choices":
                    new = Menubutton(self.optionframe, relief=RAISED, text=node.prompt)
                    ...
                    if configuration.trits_enabled:
                        w = Radiobutton(
                            ...
                            text="y",
                            variable=self.ties[node.name],
                            command=lambda x=node, self=self: self.set_symbol(x, cml.y),
                            relief=GROOVE,
                            value=cml.y,
                        )
                        ...
                        w = Radiobutton(
                            ...
                            text="m",
                            variable=self.ties[node.name],
                            command=lambda x=node, self=self: self.set_symbol(x, cml.m),
                            relief=GROOVE,
                            value=cml.m,
                        )
                        if node.type == "bool":
                        ...
                        w = Radiobutton(
                            ...
                            text="n",
                            variable=self.ties[node.name],
                            command=lambda x=node, self=self: self.set_symbol(x, cml.n),
                            relief=GROOVE,
                            value=cml.n,
                        )
                        w.pack(anchor=W, side=LEFT)
                        ...
                        w = Checkbutton(
                            ...
                            relief=GROOVE,
                            variable=self.ties[node.name],
                            command=lambda x=node, self=self: self.set_symbol(
                                x, (cml.n, cml.y)[self.ties[x.name].get()]
                            ),
                        )
                        w.pack(anchor=W, side=LEFT)
                    tw = Label(self.optionframe, text=id, relief=GROOVE, anchor=W)
                ...
                if node.name in self.ties:
                    ...
                    elif node.type in ("string", "decimal") or node.enum:
                        self.ties[node.name].set(str(node.eval()))
                    elif node.type == "hexadecimal":
                        self.ties[node.name].set("0x%x" % node.eval())
                    else:
                        enumval = node.eval()
                        if not configuration.trits_enabled and node.is_logical():
        ...
        def draw_tree(self):
            ...
            self.tree = myTree(
                self.treewindow,
                rootname=configuration.start,
                rootlabel=configuration.start.name,
                width=298,
                getcontents=get_contents,
                toggle_init=self.toggle_init,
            )
        ...
        def set_symbol(self, symbol, value):
            ...
            if not ok:
                ...
                explain += (
                    lang["ROLLBACK"] % (symbol.name, value)
                    + "\n"
    ...
    class ConfigStackMenu(ConfigMenu):
        ...
        def build(self):
            ...
            for node in menu.items:
                ...
                if node.type == "message":
                    ...
                    new = Label(
                        myframe, text=node.name + ": " + node.prompt + " = " + value
                    )
                    self.textparts[node.name] = new
                    new.config(fg="blue")
                elif node.type == "menu":
                    ...
                    new = Menubutton(myframe, relief=RAISED, text=node.prompt)
                    self.textparts[node.name] = new
                    cmenu = Menu(new, tearoff=0)
                    ...
                    for value in node.range:
                        ...
                        cmenu.add_radiobutton(
                            ...
                            value=label,
                            variable=self.ties[node.name],
                            command=lambda self=self, symbol=node, label=label: self.set_symbol(
                                symbol, label
                            ),
                        )
                    new.config(menu=cmenu)
                    ...
                    for label, value in node.range:
                        cmenu.add_radiobutton(
                            ...
                            value=value,
                            variable=self.ties[node.name],
                            command=lambda self=self, symbol=node, label=label, value=value: self.set_symbol(
                                symbol, value
                            ),
                        )
                    new.config(menu=cmenu)
                    ...
                    self.ties[node.name] = StringVar(self.workframe)
                    new = ValidatedField(
                        myframe, node, id, self.ties[node.name], self.set_symbol
                    )
                    self.textparts[node.name] = new.L
                new.pack(side=LEFT, anchor=W, fill=X, expand=YES)
        ...
        def display(self):
            menu = self.menustack[-1]
            newvisible = filter(
                lambda x, m=menu: hasattr(m, "nosuppressions")
                or interactively_visible(x),
                menu.items,
            )
            ...
            for symbol in menu.items:
                if symbol not in newvisible:
                    ...
                    if symbol.menuvalue:
                        self.symbol2widget[symbol].winfo_children()[0].config(
                            text="%s (%s)" % (symbol.prompt, symbol.menuvalue.name)
                        )
                elif symbol.discrete:
                    self.symbol2widget[symbol].winfo_children()[0].config(
                        text="%s: %s (%s)"
                        % (symbol.name, symbol.prompt, str(symbol.eval()))
                    )
...
def process_define(configuration, val, freeze):
    ...
    if sym.is_derived():
        ...
        val = parts[1]
    ok, effects, violations = configuration.set_symbol(
        sym, configuration.value_from_string(sym, val), freeze
    )
    if effects:
        print(lang["EFFECTS"])
        sys.stdout.write(string.join(effects, "\n") + "\n")
...
def main(options, arguments):
    ...
    if force_debugger:
        ...
        print(
            lang["WELCOME"] % (configuration.banner,) + lang["VERSION"] % (cml.version,)
        )
...
if __name__ == "__main__":
    try:
        ...
        if "CML2OPTIONS" in os.environ:
            envopts, envargs = getopt.getopt(os.environ["CML2OPTIONS"].split(), runopts)
    ...
    if (
        not force_tty
        and not force_debugger
        and not force_curses
        and not force_x
        and not force_q
        and not force_batch
    ):
    ...
    try:
        ...
        print(lang["ABORTED"])
        raise SystemExit(2)
    except cmlsystem.UnsatisfiableError:
