...
import sys, os, re, getopt, pickle, cml, cmlsystem
...
def makexref(tree):
    ...
    typefind = re.compile(
        r"(?<!define_)(bool|tristate|int|hex|string)\s+'.*'\s+CONFIG_(\w+)"
    )
    ...
    def xrefvisit(dict, dir, files):
        ...
        def filevisitor(dict, file):
            ...
            for match in configpref.findall(contents):
                ...
                if match in kill_list:
                    continue
                elif match not in dict:
                    dict[match] = []
...
def in_code(name):
    ...
    if name not in xrefs:
        return 0
    for file in xrefs[name]:
        if suffix(file, ".c", ".S") or (
            suffix(file, ".h") and not suffix(file, "autoconf.h")
        ):
            return 1
    return 0
...
def in_cml2(name):
    ...
    return namestrip(name in rulebase.dictionary)
...
def is_probed(name):
    ...
    if not probe_table:
        load_probe_table()
    return entry and entry.name in probe_table
    ...
    def includevisit(dummy, dir, files):
        ...
        def filevisitor(dummy, file):
            ...
            for match in configpref.findall(contents):
                ...
                if match in rulebase.dictionary and match not in matched:
...
def load_context(tree):
    ...
    if os.path.exists(xref_file):
        ...
        xrefs, mycml1types = pickle.load(ifp)
        ifp.close()
        ...
        pickle.dump((xrefs, mycml1types), ofp, 1)
        ofp.close()
        sys.stderr.write("done.\n")
...
if __name__ == "__main__":
    ...
    if genpatch:
        ...
        if shortname not in rulebase.dictionary and examine not in mycml1types:
            print("%s: no such symbol" % examine)
        else:
            print(
                "%s: a=%d c=%d h=%d o=%d n=%d m=%d d=%d x=%s f=%d"
                % (
                    examine,
                    cml1_declared(examine),
                    in_code(examine),
                    in_help(examine),
                    in_cml1(examine),
                    in_cml2(examine),
                    in_makefile(examine),
                    in_defconfig(examine),
                    is_derived(examine),
                    myfilter(examine),
                )
            )
        ...
        for key, item in mycml1types.items():
            if namestrip(key not in rulebase.dictionary):
                missing += 1
                continue
            elif len(item) != 1:
                continue
            ...
            if typemap[cml2symbol.type] == cml1type:
                ...
                cml2symbol.menu
                and cml2symbol.menu.type == "choices"
                and cml1type == "choice"
            ):
                matching += 1
            else:
