...
class Node:
    ...
    def __init__(
        self, parent, name, id, myclosed_icon, myopen_icon, x, y, parentwidget
    ):
        ...
        # add label
        self.label = self.widget.create_text(
            x + self.widget.textoff, y, text=self.name, justify="left", anchor="w"
        )
    ...
    def toggle_state(self, state=None):
        ...
        if not self.open_flag:
            ...
            if self.subnodes and self.widget.lineflag:
                self.tree = self.widget.create_line(
                    x, y, x, y + self.widget.disty * len(self.subnodes)
                )
