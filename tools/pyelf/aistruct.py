...
class BitPoker(object):
    ...
    def read_value(self, numbytes, offset_bytes):
        ...
        return struct.unpack(
            "%s%s" % (self.struct_byte_ordering, self.SIZE_MAP[numbytes]), data
        )

    def write_value_sized(self, value, numbytes, offset_bytes):
        data = struct.pack(
            "%s%s" % (self.struct_byte_ordering, self.SIZE_MAP[numbytes]), value
        )
        self.mmapfile.seek(offset_bytes + self.master_offset_bytes, 0)
    ...
    def read_value_sized(self, numbytes, offset_bytes):
        ...
        data = struct.unpack(
            "%s%s" % (self.struct_byte_ordering, self.SIZE_MAP[numbytes]), raw_data
        )
        return data[0]
    ...
    def read_c_string(self, offset_bytes):
        ...
        while c != 0:  # In Python 3, read_byte() returns int
            result += chr(c)
            c = self.mmapfile.read_byte()
        return result
...
class AIStruct(object):
    ...
    class AIElement(object):
        ...
        def write(self, bitpoker):
            bitpoker.write_value_sized(
                self.value, self.get_size_bits() // 8, self.offset
            )

        def read(self, bitpoker):
            self.value = bitpoker.read_value_sized(
                self.get_size_bits() // 8, self.offset
            )
    ...
    class BITFIELD(AIElement):
        class AttributeBasedProperty(object):
            ...
            def __setitem__(self, key, new_value):
                ...
                result |= (
                    self.bitfield_inst.value >> (offset + length) << (offset + length)
                )
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.components = []
            self.attribute_based_property = AIStruct.BITFIELD.AttributeBasedProperty(
                self
            )
    ...
    def _setup_multiprop(self, ename, times):
        def mp_get(obj):
            return [
                getattr(self.ai, "%s_%s" % (ename, counter))
    ...
    def setup(self, *elements):
        ...
        for element in elements:
            ...
            if times is not None:
                ...
                size, propdict = self._setup_one(
                    etypename, ename, args, offset, names, format
                )
                offset += size
                allprops.update(propdict)
