...
class Elf32SectionHeader(AIStruct):
    ...
    def container(self, cls):
        ...
        return ElfFileContainer(
            self.elffile.gfile,
            AIStruct.SIZE32,
            self.ai.sh_offset.get(),
            size,
            self.ai.sh_size.get() / size,
            cls,
            section=self,
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setup(
            (
                "UINT32",
                "p_type",
                {"names": {0: "NULL", 1: "LOAD", 2: "DYNAMIC", 3: "INTERP", 4: "NOTE"}},
            ),
            ("UINT32", "p_flags"),
            ("UINT64", "p_offset"),
            ("UINT64", "p_vaddr"),
    ...
    def __getitem__(self, idx):
        if type(idx) == type(""):
            ...
            if self.mutated:
                ...
                poker = BitPoker.new_with_gfile(
                    self.gfile, self.offset + (self.entsize * num)
                )
...
class ElfFileSymbolTable(object):
    ...
    def get_symbols(self, section_num=None):
        if section_num:
            return [
                sym
                for sym in self.header.container(Symbols)
                if sym.ai.st_shndx.get() == section_num
            ]
        else:
...
class ElfFile(object):
    WORD_SIZE_MAP = {
        Elf32Header.EI_CLASS_32: Elf32Header.SIZE32,
        Elf32Header.EI_CLASS_64: Elf32Header.SIZE64,
    }
    BYTE_ORDER_MAP = {Elf32Header.EI_DATA_LSB: "lsb", Elf32Header.EI_DATA_MSB: "msb"}
    HEADER_MAP = {Elf32Header.SIZE32: Elf32Header, Elf64Header.SIZE64: Elf64Header}
    PROGRAM_HEADER_MAP = {
        Elf32Header.SIZE32: Elf32ProgramHeader,
        Elf64Header.SIZE64: Elf64ProgramHeader,
    }
    SECTION_HEADER_MAP = {
        Elf32Header.SIZE32: Elf32SectionHeader,
        Elf64Header.SIZE64: Elf64SectionHeader,
    }
    ...
    def __init__(self, gfile, byte_ordering, word_size):
        ...
        self.pheaders = ElfFileProgramHeaderContainer(
            gfile,
            word_size,
            self.header.ai.e_phoff,
            self.header.ai.e_phentsize,
            self.header.ai.e_phnum.get(),
            ElfFile.PROGRAM_HEADER_MAP[word_size],
            elffile=self,
        )
        # ... section headers
        self.sheaders = ElfFileSectionHeaderContainer(
            gfile,
            word_size,
            self.header.ai.e_shoff,
            self.header.ai.e_shentsize,
            self.header.ai.e_shnum.get(),
            ElfFile.SECTION_HEADER_MAP[word_size],
            elffile=self,
        )
        # ... string table
    ...
    def from_file(filename):
        ...
        # Extract values from AIStruct objects for comparison
        magic_values = [x.get() if hasattr(x, "get") else x for x in header.ai.ei_magic]
        if magic_values != Elf32Header.EI_MAGIC:
            raise ElfFileNotElfException(
                "Wanted magic %r, got %r" % (Elf32Header.EI_MAGIC, magic_values)
            )
